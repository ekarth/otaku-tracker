...
def get_readlist_filters() -> tuple[str, str, str, str]:
...
def build_readlist_query(media_filter: str, status_filter: str, release_filter: str, book_type_filter: str):
...
@app.get("/readlist")
def index():
    media_filter, status_filter, release_filter, book_type_filter = get_readlist_filters()

    query = build_readlist_query(media_filter, status_filter, release_filter, book_type_filter)
...
@app.get("/readlist/export")
def export_readlist_csv():
    media_filter, status_filter, release_filter, book_type_filter = get_readlist_filters()
    entries = build_readlist_query(media_filter, status_filter, release_filter, book_type_filter).order_by(
    ...
    for entry in entries:
        ...
        writer.writerow(
            [
                ...
                entry.downloaded_episode_upto if entry.downloaded_episode_upto is not None else "",
